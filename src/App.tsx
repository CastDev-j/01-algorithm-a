...
const App = () => {
  ...
  const containerARef = useRef<HTMLElement>(null!);
  ...
  const changeAlphabet = (newAlphabet: string[]) => {
    const state = Flip.getState(".letter-box");
    const containerAState = Flip.getState(containerARef.current);
  ...
  return (
    <main
      className="flex flex-col items-center justify-center gap-4 px-4 max-w-4xl w-full mx-auto py-8"
      ref={containerRef}
    >
      <section
